...
\begin{figure}[H]
    ...
    \hfill
    \subfloat[Shaded by density of first hit voxel, where a low density is red and a high density results in different hue's. Overall the entire outside edge of the cloud only shows low density voxels, but there are some spots where deeper voxels are visible which have a higher density.]{
        \includegraphics[width=0.45\textwidth]{figures/disney_cloud_half_res_first_temp.png}
    }
    \caption{Some of the shading options of the VDB viewer application. The exact color values do not matter in this example, these figures exist to illustrate general patterns. All renders are done using the half resolution version of the Disney cloud \cite{DisneyCloud}. } \label{implementation:vdb_viewer:debug_views}
\end{figure}